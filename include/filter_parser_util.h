...
private:
  ...
  parse_logic_expression(const std::string &logic_expr) {
  ...
  std::vector<std::string>
  convert2rpn(const std::vector<std::string> &&oper_tokens) {
    ...
    for (const std::string &token : oper_tokens) {
      if (!is_operator(token)) {
        ...
        while (!oper_stack.empty() && oper_stack.top() != "(" &&
               higher_precedence(oper_stack.top(), token)) {
          rpn_expr.push_back(oper_stack.top());
          oper_stack.pop();
        }
        oper_stack.push(token);
  ...
  Node<T> *rpn2tree(const std::unordered_map<std::string, T> &_label_map,
...
public:
  explicit SyntaxTree<T>(T universal_label,
  ...
  bool check(bool use_universal, const std::vector<T> *labels) {
    ...
    if (use_universal) {
      return std::find(labels->begin(), labels->end(), _universal_label) !=
             labels->end();
    }
    return false;
  }
