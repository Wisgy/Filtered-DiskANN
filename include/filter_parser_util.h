...
template <typename T> class SyntaxTree
{
  private:
    ...
    Node<T> *rpn2tree(const std::vector<std::string> &&rpn, bool use_map,
                      const std::unordered_map<std::string, T> &_label_map = {})
    {
        ...
        for (auto &op : rpn)
        {
            if (!is_operator(op))
            {
                if (use_map)
                {
                    res_nodes.push(new LabelNode<T>(_label_map.at(op)));
                }
                else
                {
                    auto label = std::stoull(op);
                    res_nodes.push(new LabelNode<T>((T)label));
                }
            }
            else if (op == "|")
            {
  ...
  public:
    explicit SyntaxTree<T>(T universal_label, const std::unordered_map<std::string, T> &_label_map,
                           const std::string &str_logic_expr)
    {
        _universal_label = universal_label;
        auto tokens = parse_logic_expression(str_logic_expr);
        auto rpn = convert2rpn(std::move(tokens));
        root = rpn2tree(std::move(rpn), _label_map.size()>0 ,_label_map);
    }
    ~SyntaxTree<T>()
    {
        delete root;
    ...
    bool check(bool use_universal, const std::vector<T> *labels)
    {
        if (root->check(labels))
        {
            return true;
        }
        if (use_universal)
        {
            return std::find(labels->begin(), labels->end(), _universal_label) != labels->end();
        }
        return false;
    }
};