...
std::vector<std::vector<std::pair<uint32_t, float>>> processFilteredParts(
    const std::string &base_file, const std::string &label_file, const std::string &logic_expr,
    ...
    for (int p = 0; p < num_parts; p++)
    {
        ...
        if (logic_expr != "")
            rev_map = load_filtered_bin_as_float<T>(base_file.c_str(), base_data, npoints, dim, p, label_file.c_str(),
                                                    logic_expr, universal_label, npoints_filt, pts_to_labels);
        size_t *closest_points_part = new size_t[nqueries * k];
        float *dist_closest_points_part = new float[nqueries * k];
...
int aux_main(const std::string &base_file, const std::string &label_file, const std::string &query_file,
             ...
             const std::string &logic_expr, const std::string &tags_file = std::string(""))
{
    ...
    {
        results = processFilteredParts<T>(base_file, label_file, logic_expr, universal_label, nqueries, npoints, dim, k,
...
{
    std::string data_type, dist_fn, base_file, query_file, gt_file, tags_file, label_file, logic_expr, universal_label,
        filter_label_file;
    uint64_t K;
    ...
    try
    {
        ...
        desc.add_options()("logic_expr", po::value<std::string>(&logic_expr)->default_value(""),
        ...
        desc.add_options()("gt_file", po::value<std::string>(&gt_file)->required(),
                           ...
                           "no logic_expr or filter_label_file is provided it "
    ...
    if (logic_expr != "" && filter_label_file != "")
    {
    ...
    if (logic_expr != "")
    {
        filter_labels.push_back(logic_expr);
    }
