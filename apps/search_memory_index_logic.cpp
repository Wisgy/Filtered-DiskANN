...
#pragma omp parallel for schedule(dynamic, 1)
        for (int64_t i = 0; i < (int64_t)query_num; i++)
        {
            ...
            {
                std::string raw_logic_expr = query_filters.size() == 1 ? query_filters[0] : query_filters[i];

                auto retval = index->search_with_filters(query + i * query_aligned_dim, raw_logic_expr, recall_at, L,
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        optional_configs.add_options()("num_threads,T", po::value<uint32_t>(&num_threads)->default_value(THREADS),
